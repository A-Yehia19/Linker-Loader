...
def address():
    choose_startingAddress = Tk()
    choose_startingAddress.title("Choosing starting address")
    choose_startingAddress.geometry("400x200")

    labelll = Label(choose_startingAddress, text="plz choose starting address ya raye2", height=2,
                    font=("Berlin Sans FB", 13))
    labelll.pack()
    textinEditText = StringVar()
    # set hint (defult values)
    global edittext
    edittext = Entry(choose_startingAddress, width=8, font=("Berlin Sans FB", 13), textvariable=textinEditText)
    edittext.pack()

    btnSA = Button(choose_startingAddress, text="enter", width=9, height=2, bg="#e91e63", fg="white", borderwidth=0,
                   command=trteb_func)
    btnSA.pack()

    choose_startingAddress.mainloop()


def trteb_func():
    global startAddress
    global edittext
    startAddress = edittext.get()
    tarteeb = Tk()
    tarteeb.title("tarteeb")
    tarteeb.geometry("530x200")
    labelllll = Label(tarteeb, text="d5al tarteeb el HTE records ya raye2 w efsel benhom b spaces plz ", height=2,
                      font=("Berlin Sans FB", 13))
    labelllll.pack()

    textinEditTextt = StringVar()
    # set hint (defult values)
    global tarteebtxt
    tarteebtxt = Entry(tarteeb, width=35, font=("Berlin Sans FB", 13), textvariable=textinEditTextt)
    tarteebtxt.pack()
    btntr = Button(tarteeb, text="gamed wallahy", width=12, height=2, bg="#e91e63", fg="white", borderwidth=0,
                   command=sicXE)
    btntr.pack()

    btntr.place(x=220, y=120)
...
btnSIC.pack()
btnSIC.place(x=80, y=120)

btnSICXE = Button(ll_app, text="SICXE", width=9, height=2, bg="#e91e63", fg="white", borderwidth=0, command=address)
btnSICXE.pack()
btnSICXE.place(x=180, y=120)
################################################################
